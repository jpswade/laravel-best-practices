...
### 11. Stable Error Grouping → `rules/stable-error-grouping.md`

- Exception and log messages name the failure class only — identical for every instance of that failure
- Do not interpolate IDs, URLs, query strings, file paths, timestamps, or upstream bodies into messages
- Unique values go in `context()` on the exception, or the log context array
- Tests: two different IDs or URLs must produce the same `getMessage()`
