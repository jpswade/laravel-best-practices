...
Boost's `php/core.blade.php` already enforces "type hint everything"; this section is the natural follow-on: once the signatures are typed, the DocBlocks that duplicate them are dead weight.
...
- [`tests.md`](https://github.com/laravel/boost/blob/main/.ai/laravel/skill/laravel-best-practices/rules/tests.md) — Boost covers test mechanics; the YAGNI section above is the design counterpart (delete the code that has no test).
